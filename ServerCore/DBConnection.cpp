...
bool DBConnection::Connect(SQLHENV henv, const WCHAR* connectionString)
{
	...
	if (::SQLAllocHandle(SQL_HANDLE_STMT, _connection, OUT &_statement) != SQL_SUCCESS)
	{
		return false;
	}
...
{
	return BindParam(paramIndex, SQL_C_TYPE_TIMESTAMP, SQL_TIMESTAMP, size32(TIMESTAMP_STRUCT), value, index);
}

bool DBConnection::BindParam(SQLUSMALLINT paramIndex, const WCHAR* str, SQLLEN* index)
{
	*index = SQL_NTSL;
...
bool DBConnection::BindParam(SQLUSMALLINT paramIndex, const BYTE* bin, int32 size, SQLLEN* index)
{
	if (bin == nullptr)
	{
		*index = SQL_NULL_DATA;
...
bool DBConnection::BindCol(SQLUSMALLINT columnIndex, WCHAR* str, int32 size, SQLLEN* index)
{
